  ...
  <body>
        ...
        <div class="main-panel">
          <div class="content-wrapper">
            <div class="d-sm-flex align-items-center justify-content-between mb-4">
                <h1 class="h3 mb-0 text-gray-800">Implemented Aklan Free WiFi Sites - VSAT UNDP CoRe</h1>
            ...
            <div class="d-sm-flex align-items-center justify-content-between mb-4">
                ...
                <a href="vsat_undp_core_print.php" target="_blank" class="btn btn-md btn-primary btn-flat mr-2">
            ...
            <table class="table responsive table-striped datatable">
                ...
                <tbody>
                  <?php
                   $sql = "SELECT * FROM lezo ORDER BY priority ASC";
                    $query = $conn->query($sql);
                    while($row = $query->fetch_assoc()){
                      echo "
                        <tr>
                          <td class='hidden'></td>
                          <td>".$row['projectname_vsat_undp_core']."</td>
                          <td>".$row['barangay_vsat_undp_core']."</td>
                          <td>".$row['project_7']."</td>
                          <td>".$row['aps_vsat_undp_core']."</td>
                          <td width='50'>
    ...
    function getRow(id) {
      ...
      function getRow(id){
        $.ajax({
        type: 'POST',
        url: 'vsat_undp_core_row.php',
        data: {id:id},
        dataType: 'json',
        success: function(response){
        $('.id').val(response.id);
        $('#edit_projectname_vsat_undp_core').val(response.projectname_vsat_undp_core);
        $('#edit_barangay_vsat_undp_core').val(response.barangay_vsat_undp_core);
        $('#edit_project_7').val(response.project_7);
        $('#edit_aps_vsat_undp_core').val(response.aps_vsat_undp_core);
        $('.projectname_vsat_undp_cores').html(response.projectname_vsat_undp_core);
        }
    });
      }
    </script>
