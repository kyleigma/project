                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-12">
                      ...
                      <input type="hidden" id="project_id" name="project_id" value="<?php 
                        // Query to fetch the MIS Aklan project ID
                        $sql = "SELECT id FROM free_wifi_projects WHERE name = 'MIS Aklan' LIMIT 1";
                        $result = $conn->query($sql);
                        
                        if ($result->num_rows > 0) {
                            $row = $result->fetch_assoc();
                            echo $row['id'];  // Set MIS Aklan project ID
                        }
                      ?>">
                      <!-- Text input for displaying the project name -->
                      <input type="text" class="form-control" id="project_name" name="project_name" value="MIS Aklan" readonly>
                    </div>
                </div>
                <div class="form-group">
