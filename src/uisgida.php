...
<?php include 'includes/session.php'; ?>
  ...
  <body>
        ...
        <div class="main-panel">
          <div class="content-wrapper">
            <div class="d-sm-flex align-items-center justify-content-between mb-4">
                <h1 class="h3 mb-0 text-gray-800">Implemented Aklan Free WiFi Sites - UISGIDA</h1>
                <nav style="font-size:85%;" aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0">
                        ...
                        <li class=""><i class="mdi mdi-menu-right"></i></li>&nbsp;&nbsp;&nbsp;
                        <li class="active" aria-current="page">UISGIDA</li>
                    </ol>
                </nav>
            </div>
            <?php
            ...
            <table class="table responsive table-striped datatable">
                ...
                <tbody>
                  <?php
                    $sql = "SELECT * FROM kalibo ORDER BY priority ASC";
                    $query = $conn->query($sql);
                    while($row = $query->fetch_assoc()){
                      echo "
                        <tr>
                          <td class='hidden'></td>
                          <td>".$row['projectname_uisgida']."</td>
                          <td>".$row['barangay_uisgida']."</td>
                          <td>".$row['project_6']."</td>
                          <td>".$row['aps_uisgida']."</td>
                          <td width='50'>
    ...
    <!-- container-scroller -->
    <?php include 'includes/uisgida_modal.php';?>
    ...
    function getRow(id) {
      ...
      function getRow(id){
        $.ajax({
            ...
            $('.id').val(response.id);
            $('#edit_projectname_uisgida').val(response.projectname_uisgida);
            $('#edit_barangay_uisgida').val(response.barangay_uisgida);
            $('#edit_project_6').val(response.project_6);
            $('#edit_aps_uisgida').val(response.aps_uisgida);
            $('.projectname_uisgida').html(response.projectname_uisgida);
            }
        });
      }
    </script>
