...
<?php include 'includes/session.php'; ?>
  ...
  <body>
        ...
        <div class="main-panel">
          <div class="content-wrapper">
            <div class="d-sm-flex align-items-center justify-content-between mb-4">
                <h1 class="h3 mb-0 text-gray-800">Implemented Aklan Free WiFi Sites - Municipal</h1>
                <nav style="font-size:85%;" aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0">
                        ...
                        <li class=""><i class="mdi mdi-menu-right"></i></li>&nbsp;&nbsp;&nbsp;
                        <li class="active" aria-current="page">Municipal</li>
                    </ol>
                </nav>
            </div>
            <?php
            ...
            <div class="d-sm-flex align-items-center justify-content-between mb-4">
                ...
                <a href="municipal_print.php" target="_blank" class="btn btn-md btn-primary btn-flat mr-2">
            ...
            <table class="table responsive table-striped datatable">
                ...
                <tbody>
                  <?php
                    $sql = "SELECT * FROM batan ORDER BY priority ASC";
                    $query = $conn->query($sql);
                    while($row = $query->fetch_assoc()){
                      echo "
                        <tr>
                          <td class='hidden'></td>
                          <td>".$row['projectname_batan']."</td>
                          <td>".$row['barangay_batan']."</td>
                          <td>".$row['project_2']."</td>
                          <td>".$row['aps_batan']."</td>
                          <td width='50'>
    ...
    <!-- container-scroller -->
    <?php include 'includes/municipal_modal.php';?>
    ...
    function getRow(id) {
      ...
      function getRow(id){
        $.ajax({
          type: 'POST',
          url: 'municipal_row.php',
          data: {id:id},
          dataType: 'json',
          success: function(response){
            $('.id').val(response.id);
            $('#edit_projectname_batan').val(response.projectname_batan);
            $('#edit_barangay_batan').val(response.barangay_batan);
            $('#edit_project_2').val(response.project_2);
            $('#edit_aps_batan').val(response.aps_batan);
            $('.projectname_batan').html(response.projectname_batan);
          }
        });
      }
    </script>
