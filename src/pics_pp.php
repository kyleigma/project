...
<?php include 'includes/session.php'; ?>
  ...
  <body>
        ...
        <div class="main-panel">
          <div class="content-wrapper">
            <div class="d-sm-flex align-items-center justify-content-between mb-4">
                <h1 class="h3 mb-0 text-gray-800">Implemented Aklan Free WiFi Sites - PICS-PP</h1>
                <nav style="font-size:85%;" aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0">
                        ...
                        <li class=""><i class="mdi mdi-menu-right"></i></li>&nbsp;&nbsp;&nbsp;
                        <li class="active" aria-current="page">PICS-PP</li>
                    </ol>
                </nav>
            </div>
            <?php
            ...
            <table class="table responsive table-striped datatable">
                ...
                <tbody>
                  <?php
                    $sql = "SELECT * FROM buruanga ORDER BY priority ASC";
                    $query = $conn->query($sql);
                    while($row = $query->fetch_assoc()){
                      echo "
                        <tr>
                          <td class='hidden'></td>
                          <td>".$row['projectname_buruanga']."</td>
                          <td>".$row['barangay_buruanga']."</td>
                          <td>".$row['project_4']."</td>
                          <td>".$row['aps_buruanga']."</td>
                          <td width='50'>
    ...
    <!-- container-scroller -->
    <?php include 'includes/pics_pp_modal.php';?>
    ...
    $.ajax({
        ...
        $('.id').val(response.id);
        $('#edit_projectname_buruanga').val(response.projectname_buruanga);
        $('#edit_barangay_buruanga').val(response.barangay_buruanga);
        $('#edit_project_4').val(response.project_4);
        $('#edit_aps_buruanga').val(response.aps_buruanga);
        $('.projectname_buruanga').html(response.projectname_buruanga);
        }
    });
    }
    </script>
